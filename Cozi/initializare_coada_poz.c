...
// Functie ce initializeaza coada cu valori pozitive,elementele nu se vor repeta in coada
void initPozCoada(struct Queue *queue,int value){
    if(value >=0){
        if(queue->front == NULL){
...
int main(){
    ...
    do{
        printf("Introduceti o valoare pozitiva in coada(-1 pentr a iesii):");
        scanf("%d",&value);
        if(value > 0){
            initPozCoada(q,value);
        }
    }while(value >= 0);
